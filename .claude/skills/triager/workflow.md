...
6. **Creates a git worktree with CRDB source at the exact SHA** (enables parallel triage!)
...
The download script automatically creates a git worktree with the CockroachDB source code at the exact SHA that failed. Each issue gets its own independent source tree, allowing parallel triage. This is incredibly valuable for understanding:
...
**Test code location**: `workspace/issues/$ISSUE_NUM/cockroachdb/pkg/cmd/roachtest/tests/`

**How to find the test:**
```bash
# Example: For test "roachtest.sysbench/oltp_point_select" in issue 157102
# Look for files matching the test name
find workspace/issues/157102/cockroachdb/pkg/cmd/roachtest/tests/ -name "*sysbench*"
...
```bash
# Example: Found "context deadline exceeded" in logs for issue 157102
# Use Grep tool to search the issue's cockroachdb source:
# workspace/issues/157102/cockroachdb/pkg/
...
2. Grep the issue's CRDB source for that error message
...
1. Grep workspace/issues/157102/cockroachdb/pkg/ for "could not acquire lease"
...
With git worktrees, each issue has its own independent source tree - **no cleanup is required** between triage sessions. You can safely leave worktrees in place and triage multiple issues in parallel.

If you want to reclaim disk space, you can optionally remove a worktree:

```bash
# Remove worktree for a specific issue
...
**Note**: Deleting the entire issue workspace (`rm -rf workspace/issues/157102`) will also clean up the worktree.
